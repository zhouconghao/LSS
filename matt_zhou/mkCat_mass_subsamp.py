...
#Example submission creates Mr < 20.5 catalog based on fastphot plus add hoc e correction with a further selection of 35% lowest star formation (percentile at which data looked bimodel)
#
...
parser.add_argument("--ccut",help="a string that is used define your subsample",default='_masscut-9.0-12.1')
parser.add_argument("--zmin",help="minimum redshift for clustering catalogs",default=0.1,type=float)
parser.add_argument("--zmax",help="maximum redshift for clustering catalogs",default=0.4,type=float)
#arguments to find input data
parser.add_argument("--input_tracer", help="tracer type that subsample will come from")
parser.add_argument("--basedir", help="base directory for input, default is LSS location",default='/global/cfs/cdirs/desi/survey/catalogs/')
parser.add_argument("--outdir", help="directory for out, default is SCRATCH",default=os.environ['SCRATCH'])
parser.add_argument("--version", help="catalog version for input",default='v1.5pip')
parser.add_argument("--survey", help="e.g., Y1, DA2",default='dr1')
...
parser.add_argument("--compmd",help="use altmtl to use PROB_OBS for completeness weights in clustering catalogs",default='altmtl') # **

#what steps to run (set all to y to get NGC/SGC clustering catalogs output)
parser.add_argument("--mkfulldat", help="whether to make the initial cut file that gets used throughout",default='y')
...
parser.add_argument("--maxr", help="maximum for random files, 18 are available (use parallel script for all)",default=3,type=int) 
parser.add_argument("--nz", help="get n(z) for type and all subtypes",default='y')
parser.add_argument("--splitGC",help="convert to NGC/SGC catalogs",default='y')

#options for linear imaging systematic regressions
parser.add_argument("--imsys_clus",help="add weights for imaging systematics using eboss method, applied to clustering catalogs?",default='y')
parser.add_argument("--imsys_clus_ran",help="add weights for imaging systematics using eboss method, applied to clustering catalogs, to randoms?",default='y')
...
zmin = args.zmin #mainp.zmin
zmax = args.zmax #mainp.zmax

common.printlog("Using new script!!!!",logger)
common.printlog('zmin/zmax for clustering catalogs are '+str(zmin)+' '+str(zmax),logger)
...
common.printlog("input directory is "+dirin,logger)
common.printlog("lssmapdirout is "+lssmapdirout,logger)
common.printlog("output directory is "+args.outdir,logger)
...
# def get_FSF_loa(indata,fsf_cols,fsf_dir='/global/cfs/cdirs/desi/vac/dr2/fastphot/loa/v1.0/catalogs/',prog='bright'):
...
#         fsi = fitsio.read(fsf_dir+'fastphot-loa-main-bright-nside1-hp'+str(hp).zfill(2)+'.fits',ext='SPECPHOT',columns = fsf_cols)
#         fsl.append(fsi)
#     fs = np.concatenate(fsl)
#     del fsl
#     ol = len(indata)
#     indata = join(indata,fs,keys=['TARGETID']) #note, anything missing from fastphot will now be missing
#     del fs
#     common.printlog('length before/after fastphot join '+str(ol)+' '+str(len(indata)),logger)
#     return indata

def get_FSF_iron(indata,fsf_cols,fsf_dir='/global/cfs/cdirs/desi/vac/dr1/fastphot/iron/v1.0/catalogs/',prog='bright'):
    #add the fsf_cols to the existing data based on a TARGETID match
    #works with the data model that is new as of loa
    fsl = []
    for hp in range(0,12):
        fsi = fitsio.read(fsf_dir+'fastphot-iron-main-bright-nside1-hp'+str(hp).zfill(2)+'.fits',ext='SPECPHOT',columns = fsf_cols)
        fsl.append(fsi)
    fs = np.concatenate(fsl)
    del fsl
    ol = len(indata)
...
if args.mkfulldat == 'y':
    ...
    #for selections based on fastphot; other cases can be written similarly
    # if 'FSFABSmag' in args.ccut:
    # 'masscut-11.0-12.1'
    if 'masscut' in args.ccut:
        #this is an example that can make subsamples based on fastphot absolute magnitudes
        #other critera can be added
        csplit = args.ccut.split('-')
        
        min_mass = float(csplit[1])
        max_mass = float(csplit[2])
        fsf_cols = ['TARGETID','LOGMSTAR']
        #add more columns here based on args.ccut

        common.printlog('about to get columns from fastphot '+str(fsf_cols),logger)
        fulldat = get_FSF_iron(fulldat,fsf_cols)
        # sel = fulldat[(fulldat['LOGMSTAR'] > min_mass) & (fulldat['LOGMSTAR'] < max_mass)]
        sel = fulldat['LOGMSTAR'] > min_mass
        sel &= fulldat['LOGMSTAR'] < max_mass

        common.printlog('length after mass selection '+str(len(fulldat[sel])),logger)
...
if args.nz == 'y':
    for reg in regions:#allreg:
        ...
        extra_dir = 'nonKP'
        if args.compmd == 'altmtl':
            if args.survey == 'dr1' and args.verspec == 'iron':
                extra_dir = ""
            else:
                extra_dir = 'PIP'
