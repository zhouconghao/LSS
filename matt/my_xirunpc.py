...
def compute_angular_weights(
    ...
    **kwargs,
    ...
    (
        fibered_data_positions1,
        fibered_data_weights1,
        fibered_data_positions2,
        fibered_data_weights2,
    ) = (None, None, None, None)
    (
        parent_data_positions1,
        parent_data_weights1,
        parent_data_positions2,
        parent_data_weights2,
    ) = (None, None, None, None)
    (
        parent_randoms_positions1,
        parent_randoms_weights1,
        parent_randoms_positions2,
        parent_randoms_weights2,
    ) = (None, None, None, None)

    if mpicomm is None or mpicomm.rank == mpiroot:

        fibered_data = io.read_full_positions_weights(
            name="data", fibered=True, tracer=tracer, **catalog_kwargs
        )
        parent_data, parent_randoms = io.read_full_positions_weights(
            name=["data", "randoms"], fibered=False, tracer=tracer, **catalog_kwargs
        )
        fibered_data_positions1, fibered_data_weights1 = io.concatenate_data_randoms(
            fibered_data
        )
        (parent_data_positions1, parent_data_weights1), (
            parent_randoms_positions1,
            parent_randoms_weights1,
        ) = io.concatenate_data_randoms(parent_data, parent_randoms, **catalog_kwargs)
        if not autocorr:
            fibered_data = io.read_full_positions_weights(
                name="data", fibered=True, tracer=tracer2, **catalog_kwargs
            )
            parent_data, parent_randoms = io.read_full_positions_weights(
                name=["data", "randoms"],
                fibered=False,
            ...
            fibered_data_positions2, fibered_data_weights2 = (
                io.concatenate_data_randoms(fibered_data)
            )
            (parent_data_positions2, parent_data_weights2), (
                parent_randoms_positions2,
                parent_randoms_weights2,
            ) = io.concatenate_data_randoms(
                parent_data, parent_randoms, **catalog_kwargs
            )

    tedges = np.logspace(-4.0, 0.5, 41)
    # First D1D2_parent/D1D2_PIP angular weight
    wangD1D2 = TwoPointCorrelationFunction(
        "theta",
        tedges,
        data_positions1=fibered_data_positions1,
        data_weights1=fibered_data_weights1,
        data_positions2=fibered_data_positions2,
        data_weights2=fibered_data_weights2,
        randoms_positions1=parent_data_positions1,
        randoms_weights1=parent_data_weights1,
        randoms_positions2=parent_data_positions2,
        randoms_weights2=parent_data_weights2,
        estimator="weight",
        engine="corrfunc",
        position_type="rdd",
        nthreads=nthreads,
        ...
        mpicomm=mpicomm,
        mpiroot=mpiroot,
        weight_attrs={"normalization": "counter", "nrealizations": nreal},
    )

    # First D1R2_parent/D1R2_IIP angular weight
    # Input bitwise weights are automatically turned into IIP
    if autocorr:
        parent_randoms_positions2, parent_randoms_weights2 = (
            parent_randoms_positions1,
            parent_randoms_weights1,
        )
    wangD1R2 = TwoPointCorrelationFunction(
        "theta",
        tedges,
        data_positions1=fibered_data_positions1,
        data_weights1=fibered_data_weights1,
        data_positions2=parent_randoms_positions2,
        data_weights2=parent_randoms_weights2,
        randoms_positions1=parent_data_positions1,
        randoms_weights1=parent_data_weights1,
        randoms_positions2=parent_randoms_positions2,
        randoms_weights2=parent_randoms_weights2,
        estimator="weight",
        engine="corrfunc",
        position_type="rdd",
        nthreads=nthreads,
    ...
    wangR1D2 = None
    if not autocorr:
        wangR1D2 = TwoPointCorrelationFunction(
            "theta",
            tedges,
            data_positions1=parent_randoms_positions1,
            data_weights1=parent_randoms_weights1,
            data_positions2=fibered_data_positions2,
            data_weights2=fibered_data_weights2,
            randoms_positions1=parent_randoms_positions1,
            randoms_weights1=parent_randoms_weights1,
            randoms_positions2=parent_data_positions2,
            randoms_weights2=parent_data_weights2,
            estimator="weight",
            engine="corrfunc",
...
def compute_correlation_function(
    ...
    **kwargs,
    ...
    if mpicomm is None or mpicomm.rank == mpiroot:
        ...
        data, randoms = io.read_clustering_positions_weights(
            distance,
            name=["data", "randoms"],
            recon_dir=recon_dir,
            rec_type=rec_type,
        ...
        if (with_shifted) & (cat_read == None):
            ...
            randoms = io.read_clustering_positions_weights(
                distance,
                name="randoms",
                rec_type=False,
                tracer=tracer,
        ...
        (data_positions1, data_weights1), (randoms_positions1, randoms_weights1) = (
            io.concatenate_data_randoms(data, randoms, **catalog_kwargs)
        )
        if with_shifted:
            shifted_positions1, shifted_weights1 = io.concatenate_data_randoms(
                data, shifted, **catalog_kwargs
            )[1]
        ...
        if not autocorr:
            ...
            (data_positions2, data_weights2), (randoms_positions2, randoms_weights2) = (
                io.concatenate_data_randoms(data, randoms, **catalog_kwargs)
            )
            if with_shifted:
                shifted_positions2, shifted_weights2 = io.concatenate_data_randoms(
                    data, shifted, **catalog_kwargs
                )[1]
            jack_positions = [
                np.concatenate([p1, p2], axis=0)
                for p1, p2 in zip(jack_positions, data_positions2)
            ]

    if njack >= 2:
        subsampler = KMeansSubsampler(
            "angular",
            positions=jack_positions,
            nsamples=njack,
            nside=512,
            random_state=42,
            position_type="rdd",
    ...
    if rpcut is not None:
        selection_attrs = {"rp": (rpcut, np.inf)}
    if thetacut is not None:
        selection_attrs = {"theta": (thetacut, np.inf)}
    randoms_kwargs = dict(
        ...
        randoms_samples2=randoms_samples2,
        shifted_positions1=shifted_positions1,
        shifted_weights1=shifted_weights1,
        shifted_samples1=shifted_samples1,
        shifted_positions2=shifted_positions2,
        shifted_weights2=shifted_weights2,
    ...
    for ii, zedge in enumerate(zedges):
        if zedge.size:
            split_edges.append(
                [np.append(zedge[:, 0], zedge[-1, -1])] + list(edges[1:])
            )
    ...
    if mpicomm is None:
        ...
        nran = mpicomm.bcast(
            len(randoms_positions1) if mpicomm.rank == mpiroot else None, root=mpiroot
        )
    for i_split_randoms, edges in zip(split_randoms, split_edges):
        ...
        for iran in range(nran if i_split_randoms else 1):
            ...
            if i_split_randoms:
                ...
                for name, arrays in randoms_kwargs.items():
                    if arrays is None:
                        ...
                        arrays[0], (tuple, list)
                    ):  # e.g., list of bitwise weights
                        array = [
                            np.concatenate([arr[iarr] for arr in arrays], axis=0)
                            for iarr in range(len(arrays[0]))
                        ]
                    else:
            ...
            tmp = TwoPointCorrelationFunction(
                corr_type,
                edges,
                data_positions1=data_positions1,
...
def get_edges(corr_type="smu", bin_type="lin"):
    ...
    if bin_type == "log":
        sedges = np.geomspace(0.01, 100.0, 49)
    elif bin_type == "lin":
        sedges = np.linspace(0.0, 200, 201)
    else:
    ...
    if corr_type == "smu":
        ...
        )  # s is input edges and mu evenly spaced between -1 and 1
    elif corr_type == "rppi":
        if bin_type == "lin":
            edges = (
                sedges,
                np.linspace(-100.0, 100.0, 101),
            )  # transverse and radial separations are coded to be the same here
        else:
            edges = (sedges, np.linspace(-100.0, 100.0, 201))
            print(f"Edges = {edges}")
    elif corr_type == "theta":
        edges = (np.linspace(0.0, 4.0, 101),)
    else:
...
def corr_fn(
    ...
    #    out_dir += recon_dir+'/'
    split = (
        "_split{:.0f}".format(split_randoms_above)
        if split_randoms_above < np.inf
        else ""
    )
    wang = "{}_".format(wang) if wang is not None else ""
    root = "{}{}_{}_{}_{}_{}_njack{:d}_nran{:d}{}".format(
        wang, tracer, zmin, zmax, weight_type, bin_type, njack, nrandoms, split
    )
    if rpcut is not None:
...
if __name__ == "__main__":
    ...
    parser.add_argument(
        "--tracer",
        help="tracer(s) to be selected - 2 for cross-correlation",
        type=str,
        nargs="+",
        default=["LRG"],
    )
    ...
    parser.add_argument(
        "--basedir",
        help="where to find catalogs",
        type=str,
        default="/dvs_ro/cfs/cdirs/desi/survey/catalogs/",
    )
    parser.add_argument(
        "--survey", help="e.g., SV3, DA02, etc.", type=str, default="Y1"
    )
    parser.add_argument(
        "--verspec", help="version for redshifts", type=str, default="iron"
    )
    parser.add_argument("--version", help="catalog version", type=str, default="test")
    parser.add_argument(
        "--region",
        help="regions; by default, run on N, S; pass NS to run on concatenated N + S",
        type=str,
        nargs="*",
        choices=["N", "S", "NS", "NGC", "SGC", "NGCS", "DES", "SGCnotDES", "ALL"],
        default=None,
    )
    parser.add_argument(
        "--zlim",
        help='z-limits, or options for z-limits, e.g. "highz", "lowz", "fullonly"',
        type=str,
        nargs="*",
        default=None,
    )
    parser.add_argument(
        "--maglim",
        help="absolute r-band magnitude limits",
        type=str,
        nargs="*",
        default=None,
    )
    parser.add_argument(
        "--option", help="place to put extra options for cutting catalogs", default=None
    )
    parser.add_argument(
        "--corr_type",
        ...
        type=str,
        nargs="*",
        choices=["smu", "rppi", "theta"],
        default=["smu"],
    )
    parser.add_argument(
        "--weight_type",
        help='types of weights to use; use "default_angular_bitwise" for PIP with angular upweighting; "default" just uses WEIGHT column',
        type=str,
        default="default",
    )
    ...
    parser.add_argument(
        "--nran",
        help="number of random files to combine together (1-18 available)",
        type=int,
        default=4,
    )
    parser.add_argument(
        "--split_ran_above",
        help="separation scale above which RR are summed over each random file;\
    ...
    parser.add_argument(
        "--njack",
        help="number of jack-knife subsamples; 0 for no jack-knife error estimates",
        type=int,
        default=0,
    )
    ...
    parser.add_argument(
        "--nthreads",
        help="number of threads (defaults to 4 if --gpu else 128)",
        type=int,
        default=None,
    )
    parser.add_argument(
        "--outdir",
        help="base directory for output (default: SCRATCH)",
        type=str,
        default=None,
    )
    # parser.add_argument('--mpi', help='whether to use MPI', action='store_true', default=False)
    parser.add_argument(
        "--vis", help="show plot of each xi?", action="store_true", default=False
    )
    parser.add_argument(
        "--rebinning",
        help="whether to rebin the xi or just keep the original .npy file",
        default="y",
    )
    # arguments relevant for when running directly from full catalogs.
    parser.add_argument(
        "--use_arrays",
        help="use pre-stored arrays rather than reading from memory again",
        default="n",
    )
    parser.add_argument(
        "--write_arrays", help="save the pre-stored arrays", default="n"
    )
    # only relevant for reconstruction
    parser.add_argument(
        "--rec_type",
        help="reconstruction algorithm + reconstruction convention, but only if included in the catalog filename between dots, otherwise leave blank",
        choices=[
            "IFTPrecsym",
            "IFTPreciso",
            "IFTrecsym",
    ...
    parser.add_argument(
        "--recon_dir",
        help="if recon catalogs are in a subdirectory, put that here",
        type=str,
        default="n",
    )
    ...
    parser.add_argument(
        "--thetacut",
        help="apply the theta-cut (more up-to-date fibre collision correction), standard: 0.05",
        type=float,
        default=None,
    )
    parser.add_argument(
        "--nreal", help="number of realizations for bitweights", type=int, default=129
    )
    setup_logging()
    ...
    if args.use_arrays == "y":
        ...
        data_ = ct.mkclusdat(
            ...
            zmin=zminr,
            zmax=zmaxr,
            write_cat=write_arrays,
            return_cat="y",
        )

        randoms_ = ct.mkclusran(
            flinr,
            flinr,
            rann,
            rcols=["Z", "WEIGHT"],
        ...
        if os.path.normpath(args.basedir) == os.path.normpath(
            "/dvs_ro/cfs/cdirs/desi/survey/catalogs/"
        ):
            cat_dir = io.catalog_dir(
                base_dir=args.basedir,
                survey=args.survey,
                verspec=args.verspec,
                version=args.version,
            )
        elif os.path.normpath(args.basedir) == os.path.normpath(
            "/global/project/projectdirs/desi/users/acarnero/mtl_mock000_univ1/"
        ):
    ...
    if mpicomm is None or mpicomm.rank == mpiroot:
        logger.info(
            "Computing correlation functions {} in regions {} in redshift ranges {}.".format(
                args.corr_type, regions, zlims
            )
        )
    ...
    for zmin, zmax in zlims:
        base_file_kwargs = dict(
            ...
            njack=args.njack,
            nrandoms=args.nran,
            split_randoms_above=args.split_ran_above,
            option=option,
            rpcut=args.rpcut,
        ...
        for region in regions:
            ...
            for corr_type in args.corr_type:
                if mpicomm is None or mpicomm.rank == mpiroot:
                    logger.info(
                        "Computing correlation function {} in region {} in redshift range {}.".format(
                            corr_type, region, (zmin, zmax)
                        )
                    )
                ...
                if mpicomm is None or mpicomm.rank == mpiroot:
                    result.save(
                        corr_fn(
                            file_type="npy",
                            region=region,
                            out_dir=os.path.join(out_dir, corr_type),
                            **base_file_kwargs,
                        )
                    )
            if mpicomm is None or mpicomm.rank == mpiroot:
                        ...
                        if wang[name] is not None:
                            wang[name].save(
                                corr_fn(
                                    file_type="npy",
                                    region=region,
                                    out_dir=os.path.join(out_dir, "wang"),
                                    **base_file_kwargs,
                                    wang=name,
                                )
                            )
        ...
        for corr_type in args.corr_type:
            ...
            if mpicomm is None or mpicomm.rank == mpiroot:
                if "N" in regions and "S" in regions:  # let's combine
                    result = sum(
                        [
                            TwoPointCorrelationFunction.load(
                                corr_fn(
                                    file_type="npy",
                                    region=region,
                                    out_dir=os.path.join(out_dir, corr_type),
                ...
                if args.rebinning:
                    for region in all_regions:
                        ...
                        txt_kwargs.update(
                            region=region, out_dir=os.path.join(out_dir, corr_type)
                        )
                        result = TwoPointCorrelationFunction.load(
                            corr_fn(file_type="npy", **txt_kwargs)
                        )
                        for factor in rebinning_factors:
                            # result = TwoPointEstimator.load(fn)
                            rebinned = result[
                                : (result.shape[0] // factor) * factor : factor
                            ]
                            ...
                            if corr_type == "smu":
                                ...
                                rebinned.save_txt(
                                    fn_txt, ells=(0, 2, 4), ignore_nan=True
                                )
                                ...
                                rebinned.save_txt(fn_txt, pimax=100.0)
                                for pifac in pi_rebinning_factors:
                                    rebinned = result[
                                        : (result.shape[0] // factor) * factor : factor,
                                        : (result.shape[1] // pifac) * pifac : pifac,
                                    ]
                                    txt_kwargs.update(
                                        bin_type=args.bin_type
                                        + str(factor)
                            ...
                            if args.vis:
                                if corr_type == "smu":
                                    ...
                                    sep, xis = rebinned(
                                        pimax=100, return_sep=True, return_std=False
                                    )
                                else:
                                    sep, xis = rebinned(
                                        return_sep=True, return_std=False
                                    )
                                ...
                                if args.bin_type == "lin":
                                    for xi in xis:
                                        plt.plot(sep, sep**2 * xi)
                                tracers = tracer
                                if tracer2 is not None:
                                    tracers += " x " + tracer2
                                plt.title(
                                    "{} {:.2f} < z {:.2f} in {}".format(
                                        tracers, zmin, zmax, region
                                    )
                                )
                                plt.show()
